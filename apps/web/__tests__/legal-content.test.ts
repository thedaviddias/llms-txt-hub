...
describe('canonical legal content', () => {
  it('renders the trusted-submission privacy disclosures from the canonical collection', async () => {
    ...
    expect(privacy).toContain('Last updated: September 8, 2026')
    expect(privacy.match(/September 8, 2026/g)).toHaveLength(2)
    expect(privacy).not.toContain('August 2, 2026')
    expect(privacy).toMatch(/X or LinkedIn/i)
    expect(privacy).toMatch(/do not (?:collect|ask for|record).*username/i)
    expect(privacy).toMatch(/signed receipt tied to your signed-in account.*48 hours/i)
    ...
    expect(privacy).toMatch(/event properties.*do not include.*raw IP/i)
    expect(privacy).toMatch(/abuse-control.*HMAC-hashed source IP.*Upstash Redis/i)
