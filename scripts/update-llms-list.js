...
function generateLLMsList() {
  ...
  for (const [category, llms] of Object.entries(categorizedLLMs)) {
    ...
    llms.forEach((llm, index) => {
      ...
      // Link the name to the actual website with favicon
      llmsContent += faviconUrl
        ? `![${llm.name} favicon](${faviconUrl}) **[${llm.name}](${llm.website})**  \n`
        : `**[${llm.name}](${llm.website})**  \n`

      llmsContent += `${llm.description}  \n`

      // Add only llms.txt related links
      const links = []
      if (llm.llmsUrl) {
